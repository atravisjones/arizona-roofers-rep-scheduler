...
- **Rep switcher (v7c):** door knockers = active `rep_profiles` in sections INSURANCE / D2D with a `roofr_user_id` (`GET /api/adjuster-days?reps=1`; the old `DOOR_KNOCKER_IDS` map is gone — add a profile with a Roofr uid and the person appears). A signed-in door knocker is locked to his own day; office users get a header dropdown (`#repPick`) and land on the first door knocker. Switching (or opening a link without `s=`) fetches that rep's meetings for the date and reloads with them packed in `s=`.
