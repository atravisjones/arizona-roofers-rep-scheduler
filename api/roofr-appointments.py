...
def get_from_supabase(date_str):
    ...
    cal_path = (
        ...
        f"&category=eq.sales"
        f"&start_date=gte.{quote(date_str + ' 00:00:00')}"
        f"&start_date=lt.{quote(next_day + ' 00:00:00')}"
    ...
    job_ids = list({e["job_id"] for e in events if e.get("job_id")})
    jobs_map = {}
    if job_ids:
        ids_list = ",".join(job_ids)
        jobs = sb_fetch(f"jobs?select=job_id,customer,name,address,job_owner,workflow,tags&job_id=in.({quote(ids_list)})")
        for j in jobs:
            jobs_map[j["job_id"]] = j

    appointments = []
    for evt in events:
        job = jobs_map.get(evt.get("job_id"), {})
        appointments.append({
