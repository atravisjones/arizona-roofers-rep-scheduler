        ...
        <main className="h-full min-h-0 flex flex-col rounded-lg border border-border-primary bg-bg-primary shadow-lg overflow-hidden">
            ...
            <section className="flex-1 min-h-0 overflow-y-auto custom-scrollbar p-4">
                {isLoading ? <div className="text-sm text-text-tertiary">Loading review queue…</div> : rows.length === 0 ? <div className="text-sm text-text-tertiary">No bookings in the last 7 days.</div> : visibleRows.length === 0 ? <div className="text-sm text-text-tertiary">{tab === 'needs_review' ? 'Nothing needs review.' : 'No bookings in this view.'}</div> : <div className="flex flex-col gap-2">{visibleRows.map(row => {
                    ...
                    return <article key={row.job_id} onClick={() => setActiveJobId(row.job_id)} className={`rounded-md border bg-bg-primary px-3 py-2 space-y-1 transition ${activeJobId === row.job_id ? 'border-brand-primary ring-2 ring-brand-primary/40' : 'border-border-primary hover:border-border-secondary'} ${isRisky ? 'border-l-4 border-l-tag-amber-border' : ''}`}>
                        <div className="flex items-start justify-between gap-2"><div className="min-w-0 flex flex-wrap items-baseline gap-x-2"><h2 className="text-sm font-bold text-text-primary truncate">{row.customer || row.name || 'Unknown customer'}</h2><span className="text-[10px] text-text-tertiary whitespace-nowrap"><span className="font-semibold text-brand-primary">{formatRelativeTime(row.appt_booked_at)}</span>{' · '}{formatPhoenixDate(row.appt_booked_at)}</span></div>{isRisky && <span className="flex-shrink-0 px-1.5 py-0.5 text-[9px] font-bold rounded border border-tag-amber-border bg-tag-amber-bg text-tag-amber-text">⚠ {risks.join(', ')}</span>}</div>
                        ...
                        <div className="flex flex-wrap items-center gap-x-3 text-[11px] text-text-secondary">{row.address && <span>{row.address}</span>}{row.phone && <span>{row.phone}</span>}{row.value != null && <span className="font-semibold text-text-primary">${row.value.toLocaleString()}</span>}</div>
